...
describe("guardrails de configuração e segredos", () => {
  ...
  it("não volta a embutir a URL de projetos Supabase em arquivos de runtime", () => {
    const referenciasProjetos = [
      "cqavdefyelarhyjqmahi.supabase.co",
      "xenapowdtfhdwcfthfrn.supabase.co",
    ];
    ...
    const achados = arquivos
      ...
      .flatMap(({ arquivo, conteudo }) =>
        referenciasProjetos
          .filter((ref) => conteudo.includes(ref))
          .map((ref) => ({ arquivo, ref }))
      );
